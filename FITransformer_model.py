...
class Head(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        normalization: type=nn.LayerNorm,
        d_out: int,
    ):
        super().__init__()
...
class Transformer(nn.Module):
    ...
    This module is the backbone of `FITransformer`."""
        ...
        *,
        ...
        if (
            ...
            warnings.warn(
                ...
                'For example, the vanilla FITransformer with '
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return after_x, attentions, final_x
...
class FITransformer(nn.Module):
    ...
    Examples:
        .. testcode::
            ...
            module = FITransformer.make_baseline(
                n_num_features=3,
            ...
            module = FITransformer.make_default(
                n_num_features=3,
                cat_cardinalities=[2, 3],
                d_out=1,
            )
        ...
        .. testcode::
            ...
            baseline_parameters = FITransformer.get_baseline_transformer_subconfig()
            default_parameters = FITransformer.get_default_transformer_config()
    ...
    def __init__(
        ...
        self.Head = Head(d_in=feature_tokenizer.d_token, bias=True, activation=nn.ReLU, normalization=nn.LayerNorm, d_out=1)

        # self.total_num_features = feature_tokenizer.n_tokens
        # self.dnn_list = nn.ModuleList([SimpleDNN(input_size=feature_tokenizer.d_token, output_size=feature_tokenizer.d_token) for _ in range(self.total_num_features)])
    @classmethod
    def get_baseline_transformer_subconfig(
        cls: Type['FITransformer'],
    ) -> Dict[str, Any]:
    ...
    def get_default_transformer_config(
        cls: Type['FITransformer'], *, n_blocks: int = 3
    ) -> Dict[str, Any]:
    ...
    def make_baseline(
        ...
        *,
        ...
        """Create a "baseline" `FITransformer`.
        ...
        See `FITransformer` for usage examples.
    ...
    def make_default(
        ...
        *,
        ...
        """Create the default `FITransformer`.
        ...
        `FITransformer` for usage examples. See `FITransformer.make_baseline` for
    ...
    def forward(self, x_num: Optional[Tensor], x_cat: Optional[Tensor]) -> Tensor:
        ...
        # before_x = x
        # before_x = before_x.mean(dim=-1)

        # 特征间交互层
        after_x, attentions, x = self.transformer(x)
        after_x = after_x.mean(dim=-1)
